...
The 3D order book visual (the **fMRI BOLD Axon Tube**) is mathematically generated by combining cylindrical coordinate systems with localized physical wave equations. It bridges microstructure toxicity with biological propagation through three distinct layers:

### 1. The Tube Geometry Base (Cylindrical Mapping)

The fundamental structure of the axon tube is mapped in 3D using standard cylindrical transformations, where the radius $R(\theta, z, t)$ is modulated over time $t$, angular direction $\theta$, and axial length $z$:

$$X(\theta, z, t) = R(\theta, z, t) \cdot \cos(\theta)$$

$$Y(\theta, z, t) = R(\theta, z, t) \cdot \sin(\theta)$$

$$Z(\theta, z, t) = z$$

### 2. The Toxicity Accumulation Field (Pre-Threshold State)

Before the system breaches the critical threshold ($\tau = 0.68$), the heat field $F(z, t)$ mapping onto the tube's surface relies on the Volume-Synchronized Probability of Toxicity ($VPIN$). The depolarization concentrates near the center of the cylinder ($z_{\text{center}}$) and scales proportionally with $VPIN$:

$$F(z, t) = VPIN(t) \cdot \exp\left( - \frac{(z - z_{\text{center}})^2}{\sigma_0^2 + \gamma \cdot VPIN(t)} \right)$$

* Where $\sigma_0^2$ represents the baseline resting membrane width and $\gamma$ acts as a expansion coefficient as cellular toxicity builds up.

### 3. The Action Potential Wavefront (Post-Threshold Firing)

The instant the toxicity metric hits or breaches the boundary ($VPIN \ge \tau$), an absolute Action Potential is fired. The surface field shifts from a stationary stress concentration into a **moving Gaussian traveling wave** that sweeps down the $z$-axis over time:

$$F(z, t) = \exp\left( - \frac{(z - z_{\text{wave}}(t))^2}{\sigma_{\text{wave}}^2} \right) - R_{\text{refractory}}(z, t)$$

* **$z_{\text{wave}}(t) = z_{\text{start}} + v \cdot (t - t_{\text{fire}})$** tracking the linear forward velocity $v$ of the neon BOLD wavefront across the timeline.
* **$\sigma_{\text{wave}}^2$** represents the tight spatial focus of the active cyan firing band.
* **$R_{\text{refractory}}(z, t)$** represents a hyperpolarization/refractory decay term subtracted right behind the wave, mimicking an actual biological neuron that cannot fire twice in immediate succession as energy depletes.

### 4. Radius Deformation Matrix

To make the tube physically swell as the liquidity shock travels through it, the radius $R$ changes dynamically using the active wave field:

$$R(\theta, z, t) = R_{\text{baseline}} + \Delta R \cdot \exp\left( - \frac{(z - z_{\text{wave}}(t))^2}{\sigma_{\text{swell}}^2} \right)$$

This forces the mesh vertices to rapidly pop outward by $\Delta R$ directly underneath the moving coordinate of the BOLD wave, creating the physical "pump" effect seen in the visual engine.
---
